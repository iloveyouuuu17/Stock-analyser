...
This document outlines the architecture and steps to build a full-stack Indian Stock Sentiment Analyzer that takes a stock ticker, fetches corresponding news from ET Markets RSS feed (with a fallback to Yahoo Finance), analyzes sentiment over the headlines using finBERT, and displays a modern dashboard with stock data from yfinance.
...
*Fallback Logic*: If the filtered ET Markets RSS returns fewer than 5 headlines for the ticker, it will fetch Yahoo Finance news using `yfinance`'s `.news` property for that ticker to supplement the results.
...
Main layout wrapped in a dark theme container with the search bar, trending stocks, and dashboard display state.

#### [NEW] `frontend/src/components/Search.jsx`
- Input component to accept the NSE stock ticker (e.g., RELIANCE, TCS).
- **Trending Stocks Section**: A row of 5-6 clickable chips showing popular NSE stocks (RELIANCE, TCS, INFY, HDFCBANK, TATAMOTORS, WIPRO) for quick access. Clicking a chip sets the search query and triggers a fetch.
...
- Testing the Fallback Logic intentionally by passing an obscure ticker and asserting it falls back to Yahoo news.
- Running the frontend and making sure components render without error using `npm run dev`.
### Manual Verification
- Testing user flow in browser: Click on the "TCS" trending chip, observe the loading state, and verify the resulting dashboard contains legitimate information. Verify the UI matches a dark-themed, modern aesthetic that looks premium.
